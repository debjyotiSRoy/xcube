...
# %% auto 0
__all__ = ['BatchLbsChunkify', 'Booting']
...
class Booting:
    def __init__(self, df, bs, chnk_sz=200, device=None): store_attr('df,bs,chnk_sz,device')
    ...
    def mutual_info_gain(self, dl):
        ...
        eps = self.p_TL.new_empty(1).fill_(1e-8)
        for x,y in dl:
            ...
            self.p_TL = self.p_TL + torch.stack((p_TL_tt, p_TL_tf, p_TL_ft, p_TL_ff), dim=-1).float().sum(dim=0)
    ...
    def joint_pmf(self):
        ...
        self.p_TL_full = torch.cat(self.p_TL_full, dim=1); test_eq(self.p_TL_full.shape, (len(toks), len(lbs), 2, 2))
