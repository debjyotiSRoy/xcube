...
# %% auto 0
__all__ = ['SequentialRNN', 'AttentiveSentenceEncoder', 'AttentiveSentenceEncoder2', 'masked_concat_pool',
           'OurPoolingLinearClassifier', 'LabelAttentionClassifier', 'get_xmltext_classifier',
...
class AttentiveSentenceEncoder2(Module):
    "Create an encoder over `module` that can process a full sentence."
...
def get_xmltext_classifier2(arch, vocab_sz, n_class, seq_len=72, config=None, drop_mult=1., pad_idx=1, max_len=72*20, y_range=None):
    ...
    init = config.pop('init') if 'init' in config else None
    decoder = LabelAttentionClassifier(n_hidden, n_class, y_range=y_range)
    encoder = AttentiveSentenceEncoder2(seq_len, arch(vocab_sz, **config), decoder, pad_idx=pad_idx, max_len=max_len)
